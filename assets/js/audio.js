...
class XAudio extends HTMLElement {
  ...
  set volume(value) {
    ...
    this.refs.volumeBarSeeker.style.setProperty('--value', value);
  }
  ...
  render() {
    ...
    this.refs.time.innerHTML = `${formatSeconds(progress)} / ${formatSeconds(duration)}`;
    this.refs.stateButtonPath.setAttribute(
      'd',
      this.audio.paused ? 'M8 5.14v14l11-7l-11-7z' : 'M14 19h4V5h-4M6 19h4V5H6v14z'
    );
    this.refs.volumeButtonPath.setAttribute(
      'd',
    ...
    this.refs.progressBarSeeker.style.setProperty(
      '--value',
  ...
  initProgress() {
    ...
    slider(this.refs.progressBar);
    this.refs.progressBar.addEventListener('slider:down', down);
    this.refs.progressBar.addEventListener('slider:move', move);
    this.refs.progressBar.addEventListener('slider:up', up);
  }
    ...
    const active = new Proxy(
      { value: 0 },
      {
        set: (target, key, value) => {
          this.refs.volumeBarWrapper.classList.toggle('active', value > 0);
          return Reflect.set(target, key, value);
        },
      }
    );

    this.refs.volume.addEventListener('pointerenter', () => active.value++);
    this.refs.volume.addEventListener('pointerleave', () => active.value--);

    slider(this.refs.volumeBar);
    this.refs.volumeBar.addEventListener('slider:down', () => active.value++);
    this.refs.volumeBar.addEventListener(
      'slider:move',
      ({ detail: volume }) => (this.volume = volume)
    );
    this.refs.volumeBar.addEventListener('slider:up', () => active.value--);
  }

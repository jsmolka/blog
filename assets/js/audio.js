  ...
  <div class="audio">
    <button data-ref="stateButton">
      <svg width="16" height="16" viewBox="5 5 14 14">
        <path data-ref="stateButtonPath" fill="currentColor" d="${icon.play}" />
      </svg>
    </button>
    ...
    <div data-ref="volume" class="volume">
      ...
      <button data-ref="volumeButton">
        <svg width="16" height="16" viewBox="3 3 18 18">
          <path data-ref="volumeButtonPath" fill="currentColor" d="${icon.speaker}" />
        </svg>
      </button>
    </div>
  </div>
...
export function mount(root, src) {
  ...
  const update = () => {
    ...
    refs.time.innerHTML = `${formatSeconds(currentTime)} / ${formatSeconds(duration)}`;
    refs.stateButtonPath.setAttribute('d', audio.paused ? icon.play : icon.pause);
    refs.volumeButtonPath.setAttribute('d', muted ? icon.speakerMuted : icon.speaker);
    refs.progressBarSeeker.style.setProperty('--value', progress);
  };
